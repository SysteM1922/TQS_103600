...
public class UtilsTest {
	...
	public void testParseFromWBCurrent() {
		...
		WeatherBitData data = parseFromWBCurrent(json);
		assertEquals("Castelo Branco", data.getCity_name());
		assertEquals("PT", data.getCountry_code());
		assertEquals(39.82219, data.getLat());
		assertEquals(-7.49087, data.getLon());
		assertEquals(SearchType.CURRENT, data.getType());
	...
	public void testParseFromWBForecast() {
		...
		WeatherBitData data = parseFromWBForecast(json);
		assertEquals("Castelo Branco", data.getCity_name());
		assertEquals("PT", data.getCountry_code());
		assertEquals(39.82219, data.getLat());
		assertEquals(-7.49087, data.getLon());
		assertEquals(SearchType.FORECAST, data.getType());
		assertThat(data.getData()).isNotNull();
	}
	...
	public void testParseFromWBHistory() {
		...
		WeatherBitData data = parseFromWBHistory(json);
		assertEquals("Castelo Branco", data.getCity_name());
		assertEquals("PT", data.getCountry_code());
		assertEquals(39.82219, data.getLat());
		assertEquals(-7.49087, data.getLon());
		assertEquals(SearchType.HISTORY, data.getType());
		assertThat(data.getData()).isNotNull();
	}
